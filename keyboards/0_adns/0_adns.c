...
static void start(uint8_t address) { i2c_start(address); }
...
static void stop(void) { i2c_stop(); }
...
static void write_digits(void) { write_display(ALPHANUM_ADDRESS, alpha_display_buffer); }

static void write_matrix(void) { write_display(MATRIX_ADDRESS, matrix_display_buffer); }
...
static void write_digit_raw(uint8_t n, uint16_t bitmask) { alpha_display_buffer[n] = bitmask; }
...
static void write_digit_ascii(uint8_t n, uint8_t a, bool d) {
    ...
    if (d) alpha_display_buffer[n] |= (1 << 14);
}
